...
def generate_blog_post(cluster: Dict[str, Any], existing_titles: List[str] = None) -> Dict[str, Any]:
    ...
    for idx, sec in enumerate(sections, 1):
        ...
        section_system = (
            ...
            "   - Tables: If you generate a table, you MUST put a newline after every row (including the header and separator rows). DO NOT output the entire table on a single line. DO NOT wrap tables in markdown code blocks.\n"
            "   - Real Data Only: NEVER generate blank templates (e.g., 'Vendor A', 'Vendor B') with instructions for the user to fill them out. You are a fully autonomous SEO pipeline. If you generate a comparison table, populate it with real, plausible competitors and actual data.\n"
            ...
            "   - Subheadings: Use `###` for subheadings if necessary, but the section itself MUST start with `##`.\n"
